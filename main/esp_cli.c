...
extern void run_inference(void *image_data);
...
// NEW: Custom image inference handler
static int custom_inference_cli_handler(int argc, char *argv[])
{
    /* Just to go to the next line */
    printf("\n");
    
    if (argc != 2) {
        ESP_LOGE(TAG, "Usage: detect_custom <image_number>");
        ESP_LOGI(TAG, "Available custom images: 0 to %d", num_custom_images - 1);
        return 1;
    }
    
    int custom_image_number = atoi(argv[1]);
    
    if (custom_image_number < 0 || custom_image_number >= num_custom_images) {
        ESP_LOGE(TAG, "Image number should be between 0-%d", num_custom_images - 1);
        return 1;
    }
    
    ESP_LOGI(TAG, "Running inference on custom image %d", custom_image_number);
    
    unsigned detect_time;
    detect_time = esp_timer_get_time();
    run_inference((void *)custom_images[custom_image_number]);
    detect_time = (esp_timer_get_time() - detect_time)/1000;
    ESP_LOGI(TAG, "Time required for the inference is %u ms", detect_time);
...
int esp_cli_register_inference_command() {
    // Register standard test images command
    ...
    // Register custom images command
    esp_console_cmd_t custom_command = {
        .command = "detect_custom",
        .help = "detect_custom <image_number>"
                "Note: detect your custom images",
        .func = custom_inference_cli_handler,
    };
    esp_console_cmd_register(&custom_command);
    
    ESP_LOGI(TAG, "Registered detect_image and detect_custom commands");
    ESP_LOGI(TAG, "Custom images available: %d", num_custom_images);
